  ...
  $.extend(d3.svg.BubbleChart.prototype, {
    ...
    randomCirclesPositions: function (delta) {
      ...
      while (circles.length < self.items.length && ++interval < self.intervalMax) {
        ...
        var dist = self.innerRadius+ 20 + rad + Math.random() * (self.outerRadius - self.innerRadius - rad * 2);
        var angle = Math.random() * pi2;

        // Ajuste a un cuadrado
        if(angle >  0.7 && angle <2.28 && dist > (self.outerRadius - self.innerRadius)*0.9) // Cuadrante superior 0.78 2.35
          dist=  (self.outerRadius - self.innerRadius)*0.9;
        if(angle > 3.92 && angle <5.49 && dist > (self.outerRadius - self.innerRadius)*0.9) // Cuadrante superior 3.92 5.49
          dist=  (self.outerRadius - self.innerRadius)*0.9;
    ...
    registerClickEvent: function (node) {
      ...
      node.style("cursor", "pointer").on("click", function (d) {
        ...
        var isDiferentNode=self.clickedNode[0][0]==self.centralNode[0][0]; // agregado para verificar el click en nodos diferentes
        console.log(self.clickedNode);
        console.log(self.centralNode);
        console.log(isDiferentNode);
        self.event.send("click", self.clickedNode);
        self.reset(self.centralNode);
        self.moveToCentral(self.clickedNode);
        if(!isDiferentNode)
          self.moveToReflection(self.svg.selectAll(".node:not(.active)"), swapped); // Si se da click en el nodo central los nodos inactivos no se moverán
        swapped = !swapped;
      });
    },
